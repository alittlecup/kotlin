...
import com.example.hbl.kotlin.coderead.DirectoryNode;
...
FileCache {
    ...
    private FileCache() {
        if (hasSDCard() && hasExternalStoragePermission(CodeReadApplication.Instance)) {
            cacheDir = createFilePath(cachePath);
        } else {
            cacheDir = createFilePath(CodeReadApplication.Instance.getCacheDir() + cacheDirPath);
        }
    }
